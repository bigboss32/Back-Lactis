...
def comprobar_cuadre(p, etiqueta=""):
    ...
    for lote in p["lotes"]:
        dos = D(lote["costo_vendido"]) + D(lote["costo_en_bodega"])
        assert dos == D(lote["costo_total"]), (
            f"{etiqueta} lote {lote['fecha']}: vendido {lote['costo_vendido']} + bodega "
            f"{lote['costo_en_bodega']} = {dos}, pero el lote costó {lote['costo_total']}"
        )
        kilos = D(lote["kilos_vendidos"]) + D(lote["kilos_en_bodega"])
        ...
        # El costo del lote es la leche más su transporte
        ...
        # El detalle de la leche suma el costo del lote (si hubo leche que lo respalde)
        if not D(lote["litros_sin_recepcion"]):
        ...
        assert sum(D(v["utilidad"]) for v in lote["detalle_ventas"]) == D(lote["utilidad"])
